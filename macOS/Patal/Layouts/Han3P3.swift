//
//  Han3P3.swift
//  Patal
//
//  Created by dp on 12/25/24.
//

import Foundation

/** 자판 배열 소개
 갈마들이 공세벌식 자판 3-P 자판안 https://pat.im/1128
 */

/// enum LayoutName 의 createLayoutInstance 조합기 이름과 같아야 합니다.
struct Han3P3Layout: HangulAutomata {
    let availableTraits: Set<LayoutTrait> = [LayoutTrait.모아주기, LayoutTrait.두줄숫자, LayoutTrait.글자단위삭제, LayoutTrait.빠른마침표, LayoutTrait.옵션라틴, LayoutTrait.기호확장, LayoutTrait.ESC라틴]
    var traits: Set<LayoutTrait> = []

    /// 기호 확장 설정: 2단계 직접 조회 (오른쪽ㅗ/ㅜ → 기호키)
    /// 오른쪽 ㅗ·ㅜ를 누른 뒤 바로 기호 글쇠를 눌러 입력 (pat.im/1128 참조)
    /// 조합 중 매 키 입력마다 접근되므로 static 인스턴스를 재사용한다
    private static let _symbolExtensionConfig = SymbolExtensionConfig(
        triggerKeys: ["/", "9"],
        triggerState: .vowelOnly,
        layerKeys: [],
        symbolMap: Han3P3Layout.symbolMap
    )

    var symbolExtensionConfig: SymbolExtensionConfig? {
        can기호확장 ? Self._symbolExtensionConfig : nil
    }

    /// 기호 맵: [트리거키][기호키] → 기호 문자열
    /// 오른쪽ㅗ(/) 1단, 오른쪽ㅜ(9) 1단 (1회 누름 기준)
    /// Online-HanGeul-IME K3_P3_extended_sign_layout 기반
    private static let symbolMap: [String: [String: String]] = [
        // ── 오른쪽ㅗ (/) 1단 ──
        "/": [
            // 문장부호
            "f": "\u{00B7}",   // · 가운뎃점
            "a": "\u{2026}",   // … 말줄임표
            "g": "\u{2015}",   // ― 줄표
            "c": "\u{00B0}",   // ° 도
            "v": "\u{02D0}",   // ː 삼각콜론
            "t": "\u{3003}",   // 〃 동상부호
            "r": "\u{00B4}",   // ´ 양음부호
            "s": "\u{302F}",   // 〯 방점(거성)
            "d": "\u{302E}",   // 〮 방점(평성)
            "x": "\u{2032}",   // ′ 프라임
            "z": "\u{2033}",   // ″ 더블프라임
            "e": "\u{00D7}",   // × 곱하기
            "b": "\u{00A7}",   // § 절 기호
            ",": "\u{3001}",   // 、모점
            ".": "\u{3002}",   // 。고리점
            // 화살표
            "q": "\u{2190}",   // ← 왼쪽
            "w": "\u{2192}",   // → 오른쪽
            "1": "\u{2195}",   // ↕ 상하
            "2": "\u{2194}",   // ↔ 좌우
            // 도형
            "[": "\u{25B3}",   // △ 흰 세모
            "]": "\u{25BD}",   // ▽ 역삼각형
            "\\": "\u{00B6}",  // ¶ 단락 기호
            // 통화
            "3": "\u{FFE5}",   // ￥ 엔화
            "4": "\u{FFE6}",   // ￦ 원화
            "5": "\u{20AC}",   // € 유로
            "6": "\u{327E}",   // ㉾ 우편번호
            "`": "\u{00A9}",   // © 저작권
            "-": "\u{2642}",   // ♂ 남성
            "=": "\u{2640}",   // ♀ 여성
            ";": "\u{3265}",   // ㉥ ㅂ
            "'": "\u{326B}",   // ㉫ ㅎ
            "0": "\u{326A}",   // ㉪ ㅋ
            // 동그라미 한글 자모
            "h": "\u{3261}",   // ㉡ ㄴ
            "i": "\u{3264}",   // ㉤ ㅁ
            "j": "\u{3267}",   // ㉧ ㅇ
            "k": "\u{3260}",   // ㉠ ㄱ
            "l": "\u{3268}",   // ㉨ ㅈ
            "m": "\u{326D}",   // ㉭ ㅎ
            "n": "\u{3266}",   // ㉦ ㅅ
            "o": "\u{3269}",   // ㉩ ㅊ
            "p": "\u{326C}",   // ㉬ ㅍ
            "u": "\u{3262}",   // ㉢ ㄷ
            "y": "\u{3263}",   // ㉣ ㄹ
            // Shift 키
            "!": "\u{2921}",   // ⤡ 대각선화살표(북동)
            "@": "\u{2922}",   // ⤢ 대각선화살표(남서)
            "\"": "\u{2019}",  // ' 닫는 작은따옴표
            "H": "\u{2018}",   // ' 여는 작은따옴표
            "B": "\u{2267}",   // ≧ 크거나같음
            "G": "\u{2266}",   // ≦ 작거나같음
            "Q": "\u{2199}",   // ↙ 왼쪽아래
            "W": "\u{2198}",   // ↘ 오른쪽아래
            ":": "\u{266A}",   // ♪ 음표
            "<": "\u{2082}",   // ₂ 아래첨자2
            ">": "\u{2083}",   // ₃ 아래첨자3
            "|": "\u{00A6}",   // ¦ 파선
            // 아래첨자 숫자
            "I": "\u{2088}",   // ₈
            "J": "\u{2084}",   // ₄
            "K": "\u{2085}",   // ₅
            "L": "\u{2086}",   // ₆
            "M": "\u{2081}",   // ₁
            "N": "\u{2080}",   // ₀
            "O": "\u{2089}",   // ₉
            "U": "\u{2087}",   // ₇
        ],
        // ── 오른쪽ㅜ (9) 1단 ──
        "9": [
            // 도형
            "d": "\u{25CB}",   // ○ 흰 동그라미
            "s": "\u{25A1}",   // □ 흰 네모
            "a": "\u{25C7}",   // ◇ 흰 마름모
            "x": "\u{2606}",   // ☆ 흰 별
            "z": "\u{2661}",   // ♡ 흰 하트
            // 기호
            "e": "\u{203B}",   // ※ 참고표
            "f": "\u{300C}",   // 「 모낫괄호 열기
            "g": "\u{300D}",   // 」 모낫괄호 닫기
            "v": "\u{3008}",   // 〈 홑화살괄호 열기
            "b": "\u{3009}",   // 〉 홑화살괄호 닫기
            "r": "\u{3010}",   // 【
            "t": "\u{3011}",   // 】
            "c": "\u{260E}",   // ☎ 전화
            "y": "\u{00F7}",   // ÷ 나누기
            "0": "\u{2713}",   // ✓ 체크표시
            "'": "\u{221E}",   // ∞ 무한대
            ";": "\u{2234}",   // ∴ 그러므로
            "`": "\u{2122}",   // ™ 상표
            "w": "\u{2191}",   // ↑ 위쪽
            // 통화·단위
            "-": "\u{00B1}",   // ± 더하기빼기
            "=": "\u{2260}",   // ≠ 같지않음
            "1": "\u{03BC}",   // μ 마이크로
            "2": "\u{2109}",   // ℉ 화씨
            "3": "\u{2103}",   // ℃ 섭씨
            "4": "\u{FFE0}",   // ￠ 센트
            "5": "\u{2030}",   // ‰ 퍼밀
            "6": "\u{2702}",   // ✂ 가위
            "$": "\u{FE35}",   // ︵ 세로괄호열기
            "%": "\u{FE36}",   // ︶ 세로괄호닫기
            "<": "\u{00B2}",   // ² 위첨자2
            ">": "\u{00B3}",   // ³ 위첨자3
            "\\": "\u{2252}",  // ≒ 약간같음
            // 동그라미 숫자
            ",": "\u{2461}",   // ②
            ".": "\u{2462}",   // ③
            "m": "\u{2460}",   // ①
            "j": "\u{2463}",   // ④
            "k": "\u{2464}",   // ⑤
            "l": "\u{2465}",   // ⑥
            "u": "\u{2466}",   // ⑦
            "i": "\u{2467}",   // ⑧
            "o": "\u{2468}",   // ⑨
            "n": "\u{2469}",   // ⑩
            // Shift 키
            "Q": "\u{2193}",   // ↓ 아래쪽
            "W": "\u{2197}",   // ↗ 오른쪽위
            "B": "\u{FE40}",   // ︐ 세로쉼표
            "F": "\u{FE41}",   // ﹁ 세로모낫열기
            "G": "\u{FE42}",   // ﹂ 세로모낫닫기
            "R": "\u{FE3B}",   // ︻ 세로검은괄호열기
            "T": "\u{FE3C}",   // ︼ 세로검은괄호닫기
            "V": "\u{FE3F}",   // ︿ 세로꺾쇠열기
            // 위첨자 숫자
            "I": "\u{2078}",   // ⁸
            "J": "\u{2074}",   // ⁴
            "K": "\u{2075}",   // ⁵
            "L": "\u{2076}",   // ⁶
            "M": "\u{00B9}",   // ¹
            "N": "\u{2070}",   // ⁰
            "O": "\u{2079}",   // ⁹
            "U": "\u{2077}",   // ⁷
        ],
    ]

    // 아래 매핑들은 인스턴스와 무관한 상수이므로 static let으로 한 번만 생성하고,
    // 프로토콜 요구사항인 인스턴스 프로퍼티는 이를 그대로 위임한다.
    // (레이아웃 전환/컨트롤러 재생성 시마다 딕셔너리 리터럴이 재구성되는 것을 방지)
    private static let _chosungMap: [String: 초성] = [
        "k": 초성.기역,
        "kk": 초성.쌍기역,
        "h": 초성.니은,
        "u": 초성.디귿,
        "uu": 초성.쌍디귿,
        "y": 초성.리을,
        "i": 초성.미음,
        ";": 초성.비읍,
        ";;": 초성.쌍비읍,
        "n": 초성.시옷,
        "nn": 초성.쌍시옷,
        "j": 초성.이응,
        "l": 초성.지읒,
        "ll": 초성.쌍지읒,
        "o": 초성.치읓,
        "'": 초성.티긑,
        "0": 초성.키읔,
        "p": 초성.피읖,
        "m": 초성.히읗,
    ]
    var chosungMap: [String: 초성] { Self._chosungMap }

    private static let _jungsungMap: [String: 중성] = [
        "f": 중성.아,
        "t": 중성.애,
        "6": 중성.야,
        "T": 중성.얘,
        "r": 중성.어,
        "c": 중성.에,
        "e": 중성.여,
        "7": 중성.예,
        "v": 중성.오,
        "/": 중성.오,
        "/f": 중성.와,
        "/e": 중성.왜,
        "/d": 중성.외,
        "4": 중성.요,
        "b": 중성.우,
        "9": 중성.우,
        "9r": 중성.워,
        "9c": 중성.웨,
        "9d": 중성.위,
        "5": 중성.유,
        "g": 중성.으,
        "8": 중성.으,
        "8d": 중성.의,
        "d": 중성.이,
    ]
    var jungsungMap: [String: 중성] { Self._jungsungMap }

    private static let _jongsungMap: [String: 종성] = [
        "x": 종성.기역,
        "X": 종성.쌍기역,
        "xx": 종성.쌍기역,
        "xq": 종성.기역시옷,
        "s": 종성.니은,
        "sv": 종성.니은지읒,
        "sd": 종성.니은히읗,
        "S": 종성.니은히읗,
        "c": 종성.디귿,
        "w": 종성.리을,
        "W": 종성.리을기역,
        "wx": 종성.리을기역,
        "Z": 종성.리을미음,
        "wz": 종성.리을미음,
        "w3": 종성.리을비읍,
        "wq": 종성.리을시옷,
        "we": 종성.리을티긑,
        "wf": 종성.리을피읖,
        "wd": 종성.리을히읗,
        "Q": 종성.리을히읗,
        "z": 종성.미음,
        "3": 종성.비읍,
        "A": 종성.비읍시옷,
        "3q": 종성.비읍시옷,
        "q": 종성.시옷,
        "2": 종성.쌍시옷,
        "qq": 종성.쌍시옷,
        "a": 종성.이응,
        "v": 종성.지읒,
        "r": 종성.치읓,
        "1": 종성.키읔,
        "e": 종성.티긑,
        "f": 종성.피읖,
        "d": 종성.히흫,
    ]
    var jongsungMap: [String: 종성] { Self._jongsungMap }

    private static let _nonSyllableMap: [String: String] = [
        "Y": "/",
        "U": "7",
        "I": "8",
        "O": "9",
        "P": ";",
        "G": "<",
        "H": "\'",
        "J": "4",
        "K": "5",
        "L": "6",
        "B": ">",
        "N": "0",
        "M": "1",
        "<": "2",
        ">": "3",
    ]
    var nonSyllableMap: [String: String] { Self._nonSyllableMap }

    private static let _nonSyllableMapWith두줄숫자: [String: String] = [
        "Y": "5",
        "U": "6",
        "I": "7",
        "O": "8",
        "P": "9",
        "G": ":",
        "H": "0",
        "J": "1",
        "K": "2",
        "L": "3",
        ":": "4",
        "\"": "/",
        "B": ";",
        "N": "\'",
        "M": "\"",
    ]
    var nonSyllableMapWith두줄숫자: [String: String] { Self._nonSyllableMapWith두줄숫자 }

    func pickNonSyllable(by char: String) -> String? {
        if has두줄숫자 {
            if let nonSyllable = nonSyllableMapWith두줄숫자[char] {
                return nonSyllable
            }
        }

        guard let nonSyllable = nonSyllableMap[char] else {
            return nil
        }

        return nonSyllable
    }
}
